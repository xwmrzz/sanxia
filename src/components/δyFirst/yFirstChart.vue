<template>
  <div id="yFChart">
    ...
    <div ref="δyFirstChart" style="height:450px;width:1000px;">
    </div>
  </div>
...
<script>
  import {GetPointList,GetPointSelector} from '@/api/δyFirst/δyFirst';
  ...
  export default {
    ...
    beforeCreate() {
      GetPointSelector({
        recordStartTime: this.$store.state.δyFirst.recordStartTime,
        recordEndTime: this.$store.state.δyFirst.recordEndTime,
      }).then(res => {
    ...
    methods: {
      ...
      clear(){
        var myChart = echarts.init(this.$refs.δyFirstChart);
        myChart.dispose();
        },
      init2(){
        if (myChart != null && myChart != "" && myChart != undefined) {
          ...
          myChart = echarts.init(this.$refs.δyFirstChart);
          // 绘制图表
