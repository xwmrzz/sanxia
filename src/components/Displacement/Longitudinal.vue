  ...
  <div class="Longitudinal">
    ...
    <el-dialog
      ...
      <el-form  :model="editForm" :rules="editFormRules" ref="editForm" label-width="90px"
        ...
        <el-form-item label="MG1" prop="mg1">
          <el-input v-model="editForm.mg1" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="MG1" prop="mg2">
          <el-input v-model="editForm.mg2" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item>
...
import {
  ...
  deleteDVertival,
  listDVertical,
  updateDVertcal, updateDVertical
} from "@/api/Displacement/MonthlyLongitudinal";
import {listYVertical} from "@/api/Displacement/AnnualLongitudinal";
import {addDHorizontal, deleteDHorizontal, updateDHorizontal} from "@/api/Displacement/MonthlyLevel";
...
export default {
  ...
  created() {
    if (this.selectVal === 'day') {
      this.getMonthlyLongitudinal()
    } else {
      this.getAnnualLongitudinal()
    }
  },
  computed: {
    getTableHeight() {
  ...
  methods: {
    ...
    // 日垂直
    getMonthlyLongitudinal() {
      console.log('getMonthlyLongitudinal')
      this.tableData = []
      this.total = 0
      let Record = null
    ...
    // 年垂直
    getAnnualLongitudinal() {
      console.log('getAnnualLongitudinal')
      this.tableData = []
      this.total = 0
      let Record = null
      let recordStartTime = null
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          if (this.editForm.id || this.editForm.id === 0) {
            // 修改
            updateDVertical(this.editForm).then(res => {
            ...
            addDVertical(this.editForm).then(res => {
              console.log('add', res)
...
// 删除
    delHandle(row) {
      console.log(row)
      deleteDVertical(row.id).then(res => {
    ...
    exportExcel(){
      htmlToExcel.getExcel('#selectTable','纵向位移数据')
    }
  }
}
</script>
