<template>
  <div id="xFChart">
    ...
    <div ref="δxFirstChart" style="height:450px;width:1000px;">
    </div>
  </div>
...
<script>
  import {GetPointList,GetPointSelector} from '@/api/δxFirst/δxFirst';
  ...
  export default {
    ...
    beforeCreate() {
      GetPointSelector({
        recordStartTime: this.$store.state.δxFirst.recordStartTime,
        recordEndTime: this.$store.state.δxFirst.recordEndTime,
      }).then(res => {
    ...
    methods: {
      ...
      clear(){
        var myChart = echarts.init(this.$refs.δxFirstChart);
        myChart.dispose();
        },
      init2(){
        if (myChart != null && myChart != "" && myChart != undefined) {
          ...
          myChart = echarts.init(this.$refs.δxFirstChart);
          // 绘制图表
