  ...
  <!--        </el-form-item>-->
      ...
      <el-dialog
        ...
        <el-form :data="formData" :model="editForm" :rules="editFormRules" ref="editForm" label-width="90px"
          ...
          <el-form-item label="值" prop="level">
...
<script>
  import {GetLevel, AddLevel, DeleteLevel, UpdateLevel} from "@/api/δyFirst/δyFirst";
  ...
  export default {
    ...
    methods: {
        ...
        this.$refs[formName].validate((valid) => {
          if (valid) {
            if (this.editForm.id || this.editForm.id === 0) {
              ...
              AddLevel(this.editForm).then(res => {
                console.log('add', res)
                if (res.status === 200) {
                  this.$message({
                    showClose: true,
      ...
      exportExcel(){
        htmlToExcel.getExcel('#selectTable','地下水位数据')
      }
    }
  }
</script>
